...
def find_english_kit(english_list, kit_code):
    """根据套装编码查找英文单套预装明细（支持模糊匹配）"""
    for item in english_list:
        if item.get('套装编码') == kit_code:
            return item
    # 模糊匹配：ASM-XXXXX-NN 格式，后缀相同则匹配
    # 如 ASM-38160-01 可匹配 ASM-38294-01
...
def expand_pallet_range(pallets):
    """将托盘范围展开为单个托盘
    支持: '001#-140#', '1-141#', '01#~141#', '142#' 等格式
    """
    expanded = []
...
def get_unit_weight(item_code, english_list):
    """获取物料单重（默认25kg）"""
    return 25  # 默认25kg

def load_material_db(db_path):
    """加载物料数据库，返回 {工程编码: {工程品名_英语, 计价单位, 规格说明}} 字典"""
    if not os.path.exists(db_path):
        print(f"警告: 数据库文件 {db_path} 不存在")
        return {}
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute("SELECT 工程编码, 工程品名_英语, 计价单位, 规格说明, 重量 FROM 名称对应")
    result = {}
    for row in cur.fetchall():
        code = row[0]
        name_en = row[1] if row[1] else ''
        unit = row[2] if row[2] else ''
        spec = row[3] if row[3] else ''
        weight = row[4] if row[4] else ''
        result[code] = {'工程品名_英语': name_en, '计价单位': unit, '规格说明': spec, '重量': weight}
    conn.close()
    return result

def _lookup_db(material_db, code):
    """在数据库字典中查找，支持模糊匹配，返回 {工程品名_英语, 计价单位} 或 None"""
    ...
    for key in material_db:
        if key.startswith(code[:3]) and material_db[key]['工程品名_英语']:
            return material_db[key]
    return None

def strip_middle_b10(code):
    """去除编码中间只有B10或10的段，返回处理后的编码列表
    例如 R034-B10-2400 -> ['R034', '2400']
         SB-B10-0001 -> ['SB', '0001']
         SR-HB10-0038 -> ['SR', 'HB10', '0038'] (不变)
    """
    parts = code.split('-')
...
def get_display_code(code, db_unit=''):
    """获取用于Excel ITEM NO.列的显示编码
    单位是米的编码只保留前缀（如 R034-B10-2400 -> R034）
    其他去除中间B10后拼接（如 SB-B10-0001 -> SB-0001）
    """
...
def resolve_material_info(material_db, code):
    """
    解析物料编码，返回 (description, specification)
    规则：
      1. R034-B10-2400 → 去掉中间B10 → R034-2400 → 查R034单位=米
         → description=Rail, specification=2400
      2. SB-B10-0001 → 去掉中间B10 → SB-0001 → 查DB规格说明
         → description=Beam Splice, specification=200
      3. CR-B10-2001 → 去掉中间B10 → CR-2001 → 先查CR-2001的DB规格
         → 查不到则查前缀CR
    """
    spec = ''
...
PALLET_TYPE_MAP_EN = {
    "铁托": "Steel pallet", "木托": "Wooden pallet", "木框": "Wooden frame",
    "木箱": "Wooden crate", "纸箱": "Carton", "裸装": "Nude packing"
}

def translate_pallet_type_en(val):
    if not val:
        return val
    return PALLET_TYPE_MAP_EN.get(str(val).strip(), val)
...
def create_pallet_sheet(wb, sheet_name, data_row_count, main_mark, summary_row,
    ...
    ws['A1'] = 'SHIPPING MARK'
    ws['A1'].font = Font(name='Arial', size=48)
    ...
    ws.cell(row=2, column=1, value='MAIN MARK').font = Font(name='Arial', size=20)
    ws.cell(row=2, column=1).alignment = align_center
    ws.cell(row=2, column=2).alignment = align_center
    ws.merge_cells('C2:D2')
    ws.cell(row=2, column=3, value=f'={sn}!E2').font = Font(name='Arial', size=36)
    ...
    ws.cell(row=3, column=1, value='BATCH NO.').font = Font(name='Arial', size=20)
    ws.cell(row=3, column=1).alignment = align_center
    ws.cell(row=3, column=2).alignment = align_center
    ws.merge_cells('C3:D3')
    ws.cell(row=3, column=3, value=f'={sn}!N{summary_row}').font = Font(name='Arial', size=20)
    ...
    ws.cell(row=4, column=1, value='PROJECT NAME').font = Font(name='Arial', size=20)
    ws.cell(row=4, column=1).alignment = align_center
    ws.cell(row=4, column=2).alignment = align_center
    ws.merge_cells('C4:D4')
    ws.cell(row=4, column=3, value=f'={sn}!O{summary_row}').font = Font(name='Arial', size=20)
    ...
    ws.cell(row=5, column=1, value='PALLET NO.').font = Font(name='Arial', size=20)
    ws.cell(row=5, column=1).alignment = align_center
    ws.cell(row=5, column=2).alignment = align_center
    ws.cell(row=5, column=3, value=f'={sn}!L{summary_row}').font = Font(name='Arial', size=36)
    ws.cell(row=5, column=3).alignment = align_center
    pt_ref = f'INDEX({sn}!B:B,MATCH(C5,{sn}!L:L,0))'
    ws.cell(row=5, column=4, value=f'=IF({pt_ref}="铁托","Steel pallet",IF({pt_ref}="木托","Wooden pallet",IF({pt_ref}="木框","Wooden frame",IF({pt_ref}="木箱","Wooden crate",IF({pt_ref}="纸箱","Carton",IF({pt_ref}="裸装","Nude packing",{pt_ref}))))))').font = Font(name='Arial', size=20)
    ws.cell(row=5, column=4).alignment = align_center
    for c in range(1, 5):
        ws.cell(row=5, column=c).border = border

    ws.merge_cells('A6:B6')
    ws.cell(row=6, column=1, value='L*W*H (m)').font = Font(name='Arial', size=20)
    ws.cell(row=6, column=1).alignment = align_center
    ws.cell(row=6, column=2).alignment = align_center
    ws.merge_cells('C6:D6')
    ws.cell(row=6, column=3, value=f'={sn}!J{summary_row}').font = Font(name='Arial', size=20)
    ...
    ws.cell(row=7, column=1, value='VOLUME (㎥)').font = Font(name='Arial', size=20)
    ws.cell(row=7, column=1).alignment = align_center
    ws.cell(row=7, column=2).alignment = align_center
    ws.cell(row=7, column=3, value=f'=ROUND({sn}!K{summary_row},2)').font = Font(name='Arial', size=20)
    ws.cell(row=7, column=3).alignment = align_center
    ws.cell(row=7, column=4, value='G.W. (KGS)').font = Font(name='Arial', size=20)
    ws.cell(row=7, column=4).alignment = align_center
    ws.merge_cells('E7:F7')
    ws.cell(row=7, column=5, value=f'=ROUND({sn}!I{summary_row},2)').font = Font(name='Arial', size=20)
    ws.cell(row=7, column=5).alignment = align_center
    for c in range(1, 7):
        ws.cell(row=7, column=c).border = border

    headers = ['NO.', 'ITEM NO.', 'DESCRIPTION', 'SPECIFICATION', 'QTY. (PCS)', 'REMARK']
    ...
    for seq in range(1, data_row_count + 1):
        r = 8 + seq
        ws.cell(row=r, column=1, value=f'No.{seq}').font = Font(name='Arial', size=20)
        ws.cell(row=r, column=2, value=f'=VLOOKUP($C$5&"-No."&{seq},{vlookup_range},2,0)').font = Font(name='Arial', size=20)
        ws.cell(row=r, column=3, value=f'=VLOOKUP($C$5&"-No."&{seq},{vlookup_range},3,0)').font = Font(name='Arial', size=20)
        ws.cell(row=r, column=4, value=f'=VLOOKUP($C$5&"-No."&{seq},{vlookup_range},4,0)').font = Font(name='Arial', size=20)
        ws.cell(row=r, column=5, value=f'=VLOOKUP($C$5&"-No."&{seq},{vlookup_range},5,0)').font = Font(name='Arial', size=20)
        ws.cell(row=r, column=6, value='').font = Font(name='Arial', size=20)
        for c in range(1, 7):
            ws.cell(row=r, column=c).border = border
    ...
    ws.cell(row=mic_row, column=1).font = Font(name='Arial', size=30, bold=True)
...
def convert_pallet_to_rows(pallet, chinese_list, english_list, material_db):
    """将一个托盘的数据转换为多行Excel数据，返回 (rows_data, kit_groups)"""
    rows_data = []
...
def create_excel_from_pallets(pallet_json_path, output_file_path, 
    ...
    for row_data in all_rows:
        ...
        # F列: QTY = parent_qty * per_unit_qty (套装行留空)
        ...
        # H列: N.W. (合并后由pallet range写入SUM公式)
        # I列: G.W. (合并后由pallet range写入公式)
        # J列: L*W*H
    ...
    # 套装子物料组外层红色边框（medium style，必须在合并单元格之后，仅C~G列）
    ...
    for row in range(2, last_row + 1):
        for col in range(3, 16):  # C=3, O=15
            cell = ws.cell(row=row, column=col)
            # 创建新的边框，保留原有边框，添加外围边框
            new_border = Border(
    ...
    # ========== 拆分：为每个托盘生成独立工作表 ==========
    logo_path = os.path.join(os.path.dirname(output_file_path), 'logo_en.png')
    ...
    print(f"\n{'='*60}")
    print(f"[OK] Excel 文件 '{output_file_path}' 已生成!")
    print(f"[STAT] 共处理 {len(pallets)} 个托盘，{total_rows_count} 行物料明细数据。")
    print(f"\n[SUMMARY]")
    print(f"  QTY (PCS): {total_qty}")
    print(f"  PALLETS: {total_rows_count}")
    print(f"{'='*60}")
