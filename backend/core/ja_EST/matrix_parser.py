...
def _extract_arrays(df):
    ...
    array_end = min(array_start + 120, df.shape[0])
    consecutive_empty = 0
    ...
    for r in range(array_start, array_end):
        ...
        _inv_keywords = ('汇流箱', 'パワコン', '逆变器', '集電箱', '集电箱', 'PB')
        for c in range(min(3, df.shape[1])):
            val = _safe_str(df.iat[r, c])
            if any(kw in val for kw in _inv_keywords):
                has_inverter = True
                break
        ...
        if note_col is not None and note_col < df.shape[1]:
            ...
            if val and not val.replace('.', '').replace('-', '').isdigit():
                note = val
                if not has_inverter:
                    _inv_keywords = ('汇流箱', 'パワコン', '逆变器', '集電箱', '集电箱', 'PB', 'インバータ')
                    has_inverter = any(kw in note for kw in _inv_keywords)
        else:
            for c in [8, 9]:
                if c >= df.shape[1]:
                    continue
