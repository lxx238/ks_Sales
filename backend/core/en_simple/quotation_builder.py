...
from backend.core.en_simple.quotation_engine import (
    create_detail_sheet,
...
def split_and_create_quotations(
    ...
    if arrays:
        ...
        print(f"   [EN] matrix has {len(arrays)} arrays ({len(valid_arrays)} valid): {[(a.get('rows'), a.get('cols'), a.get('table_qty')) for a in valid_arrays]}")
    else:
        print(f"   [EN] WARNING: no arrays in matrix_data, matrix_data keys={list((matrix_data or {}).keys())}")
    all_detail_results = []
    all_unmatched_products = []

    used_array_indices = set()
    _match_skip_count = 0
    _match_success_count = 0
    ...
    for key, products in products_by_key.items():
        ...
        if not bom_info:
            print(f"   [EN] skip {key}: no bom_info")
            continue
        ...
        if matched_array is None:
            _match_skip_count += 1
            print(f"   [EN] skip {key}: array='{bom_array_str}' rows={bom_rows} cols={bom_cols} base={config.get('base_count',0)} no match in {len(arrays)} arrays")
            continue
        _match_success_count += 1
        ...
        try:
            detail = create_detail_sheet(
                ...
                span_info=span_info_by_key.get(key, ''),
            )
    ...
    if all_detail_results:
        try:
            create_summary_sheet(
                ...
                skip_freight=(trade_method in ('EXW', 'FAC')),
        ...
        print(f"\n   no detail sheets generated (products_by_key={len(products_by_key)}, "
              f"arrays={len(arrays)}, matched={_match_success_count}, skipped={_match_skip_count})")
        if not arrays:
            print(f"   [EN] HINT: no info sheet arrays - check if matrix file was uploaded correctly")
        elif _match_skip_count > 0 and _match_success_count == 0:
            print(f"   [EN] HINT: all BOM arrays mismatch info sheet - check BOM/info sheet compatibility")
            bom_arrays_summary = []
            for _bk, _bp in products_by_key.items():
                _bbi = bom_info_by_key.get(_bk)
                if _bbi:
                    _bcfg = _bbi.get('config', {})
                    bom_arrays_summary.append(f"{_bcfg.get('array', '?')}(base={_bcfg.get('base_count', 0)})")
            print(f"   [EN] BOM arrays: {bom_arrays_summary}")
            info_arrays_summary = [(a.get('rows'), a.get('cols'), a.get('table_qty')) for a in arrays if (a.get('table_qty') or 0) > 0]
            print(f"   [EN] Info arrays: {info_arrays_summary}")
        if image_temp_dir and os.path.exists(image_temp_dir):
            try:
                shutil.rmtree(image_temp_dir)
            except Exception:
