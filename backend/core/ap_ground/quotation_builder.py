...
按 group='亚太组' 且 ap_case_type=='GROUND' 路由调用。

流水线：
    BOM 解析（标准 V3.0，复用 shared.bom_utils）→ 信息表阵列匹配（rows×cols）
    → 每阵列 collect + 明细页 → 汇总页 Sheet1 → 保存输出。

与 ap_common（屋顶）/ en_simple 完全隔离，输出报价表版式对齐 006 地面样例。
"""
...
from backend.core.ap_ground.quotation_engine import (
    collect_ap_ground_array,
    create_ap_ground_detail_sheet,
    create_ap_ground_summary_sheet,
)
...
def split_and_create_quotations(
    ...
    if latest_log:
        ...
        if log_mapping:
            ...
            print(f"[AP-GROUND] loaded image log: {latest_log}")
    ...
    print(f"\n{'=' * 80}\n[AP-GROUND] Processing: {input_file}\n{'=' * 80}\n")
    ...
    print(f"   [AP-GROUND] matrix has {len(arrays)} arrays ({len(valid_arrays)} valid): "
        ...
        try:
            ...
            if _zip_result and _zip_result[0]:
                ...
                if _valid >= len(_zi):
                    ...
                    use_zip = True
                    print(f"[AP-GROUND] ZIP fast parsing: {len(products_by_key)} BOM entries")
        except Exception as _e:
            print(f"[AP-GROUND] ZIP parse failed ({_e}), fallback to full parse")
    ...
    for key, products in products_by_key.items():
        ...
        if not bom_info:
            print(f"   [AP-GROUND] skip {key}: no bom_info")
            continue
        ...
        if matched_array is None:
            match_skip += 1
            print(f"   [AP-GROUND] skip {key}: array='{bom_array_str}' no match")
            continue
        match_success += 1
        ...
        try:
            collected = collect_ap_ground_array(
            ...
            detail = create_ap_ground_detail_sheet(
            ...
            import traceback
            print(f"   [AP-GROUND] detail failed: {e}")
            traceback.print_exc()
    ...
    if all_unmatched_products:
        try:
            ...
            if inquiry_file:
                print(f"[AP-GROUND] Inquiry: {inquiry_file}")
        except Exception as e:
            print(f"   [AP-GROUND] inquiry sheet failed: {e}")
    ...
    if all_detail_results:
        try:
            create_ap_ground_summary_sheet(
                ...
                ap_special_discount_rate=ap_special_discount_rate,
                module_wattage=module_wattage, image_path=image_path,
                container_details=container_details,
            )
        except Exception as e:
            import traceback
            print(f"   [AP-GROUND] summary sheet failed: {e}")
            traceback.print_exc()

        input_basename = os.path.splitext(os.path.basename(input_file))[0]
        output_file = os.path.join(output_dir, f"{input_basename}_地面报价.xlsx")
        set_page_break_preview(master_wb)
        master_wb.save(output_file)
        print(f"[AP-GROUND] Saved → {output_file}")
    ...
    if image_temp_dir and os.path.exists(image_temp_dir):
        try:
            ...
            pass
    print(f"[AP-GROUND] no detail sheets generated (matched={match_success}, skipped={match_skip})")
    return None
