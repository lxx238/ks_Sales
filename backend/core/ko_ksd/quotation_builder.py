...
from backend.core.ko_ksd.quotation_engine import (
...
def split_and_create_quotations(
    ...
    if latest_log:
        ...
        if log_mapping:
            ...
            print(f"[KSD] loaded image log: {latest_log}")
    ...
    print(f"\n{'=' * 80}")
    print(f"[KSD] Processing: {input_file}")
    print(f"{'=' * 80}\n")
        ...
        try:
            ...
            if _zip_result and _zip_result[0]:
                ...
                if _valid_arrays < len(zip_bom_info_by_key):
                    print(f"[KSD] ZIP parsing: only {_valid_arrays}/{len(zip_bom_info_by_key)} "
                          f"BOMs have valid arrays, falling back to non-ZIP")
                    zip_products_by_key = None
                    zip_bom_info_by_key = None
                else:
                    use_zip = True
                    print(f"[KSD] ZIP fast parsing: {len(zip_products_by_key)} BOM entries")
        except Exception:
            pass
    ...
    if has_explicit_matrix_arrays and matrix_array_entries:
        ...
        if _sheet_match_counts:
            ...
            print(f"[KSD] Sheet order by match count: {[(sn, _sheet_match_counts.get(sn, 0)) for sn in bom_sheet_names if sn in _sheet_match_counts]}")
    ...
    for sheet_name in bom_sheet_names:
        ...
        print(f"[KSD] Sheet: {sheet_name} ({df.shape})")
    ...
    if pending_boms and has_explicit_matrix_arrays and matrix_array_entries:
        ...
        for acc in info_acc_results:
            ...
            try:
                ...
                import traceback
                print(f"   ❌ KSD info-accumulated detail failed: {e}")
                traceback.print_exc()
    ...
    if pending_boms and has_explicit_matrix_arrays and matrix_array_entries:
        ...
        for m_idx in remaining_info_indices:
            ...
            try:
                ...
                import traceback
                print(f"   ❌ KSD indirect detail failed: {e}")
                traceback.print_exc()
    ...
    if pile_products_all:
        ...
        print(f"   🔩 KSD地桩汇总: 数量={_pq}, 金額={_pa:.2f}")

    if all_quotation_results and has_explicit_matrix_arrays and matrix_array_entries:
        reorder_sheets_by_matrix_array(master_wb, all_quotation_results, matrix_array_entries, log_prefix='[KSD]')
    ...
    if need_total_materials and all_quotation_results:
        try:
            ...
            import traceback
            print(f"   ❌ KSD total materials sheet failed: {e}")
            traceback.print_exc()
    ...
    if all_quotation_results:
        ...
        print(f"\n[KSD] Done! {len(all_quotation_results)} sheets → {output_file}")
        ...
        if all_unmatched_products:
            ...
            if inquiry_file:
                print(f"[KSD] Inquiry: {inquiry_file}")
        else:
            inquiry_file = None
