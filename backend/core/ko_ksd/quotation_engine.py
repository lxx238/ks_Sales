...
def _render_subtable(ws, start_row, products, price_mapping, col_count=12, currency_label='US$'):
    ...
    for idx, product in enumerate(products):
        ...
        try:
            ...
            ws.cell(row=row, column=5, value=_strip_cjk_spec(product.get('spec', ''))).alignment = center
        if display_price > 0:
...
def create_ksd_detail_sheet(
    ...
    print(f"[KSD] Creating detail sheet: {sheet_name}")
    ...
    if need_weight_code:
        ...
        column_widths = {
            'A': 15.77, 'B': 28.67, 'C': 42, 'D': 30.03, 'E': 34.58,
    ...
    ws['A2'] = matrix_project_name or 'Solar mounting system Quotation'
        ...
        for idx, product in enumerate(products):
            ...
            if need_weight_code:
                ...
                if unit_weight is not None and unit_weight > 0:
                    ...
                    bom_w = product.get('weight', 0)
                    print(f"   [KSD-WEIGHT] row={row} code={product_code} qty={quantity} "
    ...
    def _write_group_totals(ws, row, sub_total, total_all, kw, sub_weight=0, total_weight=0, data_first_row=None, data_last_row=None):
        ...
        if total_table_count > 1:
            ws.cell(row=row, column=8, value=f'=H{sub_total_row}*{total_table_count}')
        else:
        ...
        if need_weight_code:
            if total_table_count > 1:
                ws.cell(row=row, column=9, value=f'=I{sub_total_row}*{total_table_count}')
            else:
    ...
    part1_inq_price = 0.0
    if aluminum:
        _write_part_title(ws, current_row, 'Part 1: Aluminum/Stainless Products')
        current_row += 1
        ...
        (data_end, part1_sub_total, part1_sub_weight,
         ...
         part1_std_price, part1_steel_price, part1_purchased_price, part1_inq_price,
          _) = _write_product_rows(ws, current_row, aluminum, price_mapping)
    ...
    part2_sub_total = 0.0
    part2_total_all = 0.0
    part2_sub_weight = 0.0
    part2_total_weight = 0.0
    ...
    part2_std_price = 0.0
    part2_steel_price = 0.0
    part2_purchased_price = 0.0
    part2_inq_price = 0.0
    is_complex = False
    if carbon_steel:
        _write_part_title(ws, current_row, 'Part 2: Carbon Steel Products')
        current_row += 1
        _write_group_header(ws, current_row)
        current_row += 1
        part2_data_start = current_row
        (data_end, part2_sub_total, part2_sub_weight,
         part2_std_rows, part2_steel_rows, part2_purchased_rows, part2_inq_rows,
         part2_unmatched_rows,
         part2_std_price, part2_steel_price, part2_purchased_price, part2_inq_price,
          is_complex) = _write_product_rows(ws, current_row, carbon_steel, price_mapping)
        part2_data_end = data_end
        current_row = data_end + 1
        part2_total_all = part2_sub_total * total_table_count
        part2_total_weight = part2_sub_weight
        part2_total_table_row = current_row
        current_row = _write_group_totals(ws, current_row, part2_sub_total, part2_total_all, output_kw, part2_sub_weight, part2_total_weight, data_first_row=part2_data_start, data_last_row=part2_data_end) + 1
    ...
    print(f"   [KSD] Images: found={image_found_count}, not_found={image_not_found_count}")
    ...
    for p in aluminum + carbon_steel + excluded:
    ...
    return {
        ...
        'valid_products': len(aluminum) + len(carbon_steel) + len(excluded),
        'total_weight': 0,
        ...
        'part2_price_per_table': part2_sub_total,
        'part2_price_all_tables': part2_total_all,
        'part2_total_table_row': part2_total_table_row,
        'part2_standard_rows': part2_std_rows,
        'part2_steel_rows': part2_steel_rows,
        'part2_purchased_rows': part2_purchased_rows,
        'part2_inquiry_rows': part2_inq_rows,
        'part2_unmatched_rows': part2_unmatched_rows,
        'is_complex': is_complex,
        'part2_standard_total': part2_std_price,
        'part2_steel_total': part2_steel_price,
        'part2_purchased_total': part2_purchased_price,
        'part2_inquiry_total': part2_inq_price,
...
def create_ksd_summary_sheet(
    ...
    for col_letter in 'ABCDEFGHIJKLM':
        if col_letter in ('C', 'D', 'E', 'F'):
            ws.column_dimensions[col_letter].width = 15
        else:
            ws.column_dimensions[col_letter].width = 8

    ws['K1'] = 'Date:'
    ws['K1'].font = normal_font
    ws['K1'].alignment = right_a
    ws.merge_cells('L1:M1')
    ws['L1'] = datetime.now().strftime('%Y/%m/%d')
    ws['L1'].font = normal_font
    ws['L1'].alignment = left_bottom
    ...
    part2_total_all = Decimal('0')
    part2_per_table = Decimal('0')
    part1_std_total = Decimal('0')
    part1_steel_total = Decimal('0')
    part1_purchased_total = Decimal('0')
    part1_inq_total = Decimal('0')
    part2_std_total = Decimal('0')
    part2_steel_total = Decimal('0')
    part2_purchased_total = Decimal('0')
    part2_inq_total = Decimal('0')
    ...
    for qr in all_quotation_results:
        ...
        part2_total_all += Decimal(str(qr.get('part2_price_all_tables', 0)))
        part2_per_table += Decimal(str(qr.get('part2_price_per_table', 0)))
        ...
        part2_std_total += Decimal(str(qr.get('part2_standard_total', 0)))
        part2_steel_total += Decimal(str(qr.get('part2_steel_total', 0)))
        part2_purchased_total += Decimal(str(qr.get('part2_purchased_total', 0)))
        part2_inq_total += Decimal(str(qr.get('part2_inquiry_total', 0)))
    ...
    part1_discounted = (float(part1_std_total) * ko_discount_rate / 100.0
    ...
    part2_discounted = (float(part2_std_total) * ko_discount_rate / 100.0
                        + float(part2_steel_total) * ko_steel_discount_rate / 100.0
                        + float(part2_purchased_total) * ko_purchased_discount_rate / 100.0
                        + float(part2_inq_total))
    grand_total = part1_discounted + part2_discounted
    unit_per_w = round(grand_total / (float(total_output_kw) * 1000), 7) if total_output_kw > 0 else 0

    has_part2 = any(qr.get('part2_price_per_table', 0) for qr in all_quotation_results)
    ...
    ws.merge_cells(f'A{part1_start}:I{part1_start}')
    ...
    part1_headers = ['NO.', 'Row', 'Column', 'Installation Angle', '', 'Table', 'Less', 'Power（kw）', '', f'Price({ksd_currency_label}/Table)', '', f'Amount({ksd_currency_label})', '']
    ...
    for i, qr in enumerate(all_quotation_results):
        ...
        if _acc_gid:
            ...
            r_miss = _first_ma.get('missing_per_table', 0) or 0
            ws.cell(row=r, column=6, value=_total_base).alignment = center
            ws.cell(row=r, column=7, value=r_miss if r_miss != 0 else '/').alignment = center
            ...
            ws.cell(row=r, column=4, value=angle_display).alignment = center
            ws.merge_cells(f'D{r}:E{r}')
            r_miss = ma.get('missing_per_table', 0) or 0
    ...
    ws.cell(row=total_row, column=8, value=f'=SUM(H{data_row}:H{data_end})').alignment = center
    ...
    ws.cell(row=disc_p1_row, column=12, value=f'=L{total_row}*{ko_discount_rate}/100').number_format = ksd_currency_fmt
    ws.cell(row=disc_p1_row, column=12).font = red_bold_font
    ...
    ws.cell(row=perw_row, column=12, value=f'=ROUND(IF(H{total_row}>0,L{disc_p1_row}/(H{total_row}*1000),0),3)').number_format = '#,##0.000'
    ...
    perw2_row = None
    if has_part2:
        part2_start = perw_row + 1
        part2_title_font = Font(name='Calibri', size=12, bold=True)
        ws.merge_cells(f'A{part2_start}:I{part2_start}')
        ws.cell(row=part2_start, column=1, value='Part 2:Carbon steel horizontal rail').font = part2_title_font
        ws.row_dimensions[part2_start].height = 25

        hdr2_row = part2_start + 1
        part2_headers = ['NO.', 'Row', 'Column', 'Installation Angle', '', 'Table', 'Less', 'Power（kw）', '', f'Price({ksd_currency_label}/Table)', '', f'Amount({ksd_currency_label})', '']
        for ci, h in enumerate(part2_headers):
            cell = ws.cell(row=hdr2_row, column=ci + 1, value=h)
            cell.font = bold_font
            cell.alignment = center
            cell.border = thin_border
        ws.merge_cells(f'D{hdr2_row}:E{hdr2_row}')
        ws.merge_cells(f'H{hdr2_row}:I{hdr2_row}')
        ws.merge_cells(f'J{hdr2_row}:K{hdr2_row}')
        ws.merge_cells(f'L{hdr2_row}:M{hdr2_row}')
        ws.row_dimensions[hdr2_row].height = 30

        data2_row = hdr2_row + 1
        for i, qr in enumerate(all_quotation_results):
            r = data2_row + i
            cfg = qr.get('config') or {}
            md = qr.get('matrix_data') or matrix_data
            ...
            r_kw = md.get('output_kw') or 0
            detail_sheet = qr.get('sheet_name', '')
            p2_total_table_row = qr.get('part2_total_table_row', 0)
            ...
            ws.merge_cells(f'D{r}:E{r}')
            ws.cell(row=r, column=6, value=r_set or '').alignment = center
            ws.cell(row=r, column=7, value='').alignment = center
            ws.cell(row=r, column=8, value=r_kw).alignment = center
            ws.merge_cells(f'H{r}:I{r}')
            ...
            for ci in range(1, 14):
                ws.cell(row=r, column=ci).font = normal_font
                ws.cell(row=r, column=ci).border = thin_border
            if not qr.get('part2_price_per_table', 0):
                for ci in range(1, 14):
                    ws.cell(row=r, column=ci).fill = yellow_fill
            ws.row_dimensions[r].height = 25

        data2_end = data2_row + len(all_quotation_results) - 1

        total2_row = data2_end + 1
        ws.merge_cells(f'A{total2_row}:G{total2_row}')
        ws.cell(row=total2_row, column=1, value='Total').font = bold_font
        ws.cell(row=total2_row, column=1).alignment = center
        ws.merge_cells(f'H{total2_row}:I{total2_row}')
        ws.cell(row=total2_row, column=8, value=f'=SUM(H{data2_row}:H{data2_end})').alignment = center
        ws.merge_cells(f'J{total2_row}:K{total2_row}')
        ws.merge_cells(f'L{total2_row}:M{total2_row}')
        ws.cell(row=total2_row, column=12, value=f'=SUM(L{data2_row}:L{data2_end})').number_format = ksd_currency_fmt
        ws.cell(row=total2_row, column=12).alignment = center
        for ci in range(1, 14):
            ws.cell(row=total2_row, column=ci).border = thin_border

        disc_p2_row = total2_row + 1
        ws.merge_cells(f'A{disc_p2_row}:K{disc_p2_row}')
        ws.cell(row=disc_p2_row, column=1, value='DISCOUNT').font = red_bold_font
        ws.cell(row=disc_p2_row, column=1).alignment = right_a
        ws.merge_cells(f'L{disc_p2_row}:M{disc_p2_row}')
        ws.cell(row=disc_p2_row, column=12, value=f'=L{total2_row}*{ko_steel_discount_rate}/100').number_format = ksd_currency_fmt
        ws.cell(row=disc_p2_row, column=12).font = red_bold_font
        ws.cell(row=disc_p2_row, column=12).alignment = center
        for ci in range(1, 14):
            ws.cell(row=disc_p2_row, column=ci).border = thin_border

        perw2_row = disc_p2_row + 1
        ws.merge_cells(f'A{perw2_row}:K{perw2_row}')
        ws.cell(row=perw2_row, column=1, value='per/w').font = bold_font
        ws.cell(row=perw2_row, column=1).alignment = right_a
        ws.merge_cells(f'L{perw2_row}:M{perw2_row}')
        ws.cell(row=perw2_row, column=12, value=f'=ROUND(IF(H{total2_row}>0,L{disc_p2_row}/(H{total2_row}*1000),0),3)').number_format = '#,##0.000'
        ws.cell(row=perw2_row, column=12).alignment = center
        for ci in range(1, 14):
            ws.cell(row=perw2_row, column=ci).border = thin_border

    last_part_row = perw2_row if has_part2 else perw_row

    if disc_p2_row:
        ws.cell(row=10, column=3, value=f'=ROUND(IF(H{total_row}>0,(L{disc_p1_row}+L{disc_p2_row})/(H{total_row}*1000),0),3)').number_format = '#,##0.000'
    else:
        ws.cell(row=10, column=3, value=f'=ROUND(IF(H{total_row}>0,L{disc_p1_row}/(H{total_row}*1000),0),3)').number_format = '#,##0.000'

    hint_row = last_part_row + 2
    ws.merge_cells(f'A{hint_row}:M{hint_row}')
    ...
    print(f"   ✅ KSD summary sheet(Total) created")
    return ws.title
