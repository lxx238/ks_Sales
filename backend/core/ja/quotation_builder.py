...
def _build_products_by_key(input_file, selected_key_set, column_mapping, skip_keywords, non_bom_sheet_keywords):
    ...
    for sheet_name in bom_sheet_names:
        ...
        for original_index, bom_info in enumerate(bom_starts, 1):
            ...
            if not sheet_inv_extracted:
                inv_codes, inv_rows, sheet_inv_total = _find_inverter_region(df, bom_info)
                if not inv_rows:
                    inv_codes, inv_rows = _find_inverter_region_by_name(df)
                if inv_rows:
                    ...
                    sheet_inv_rows = inv_rows
                sheet_inv_extracted = True
            ...
            if not products:
                continue
            inv_row_set = set(sheet_inv_rows)
...
def split_and_create_quotations(
    ...
    def _process_products_by_key(products_by_key, bom_info_by_key, master_wb):
        for key, products in products_by_key.items():
            ...
            sheet_prefix = f"{matched_rows}×{matched_cols}_{matched_qty}"
            ...
            detail_sheet_products.extend(bracket_prods)
            if pile_prods:
                for pp in pile_prods:
                    _pp_copy = dict(pp)
    ...
    if pending_boms and has_explicit_matrix_arrays and matrix_array_entries:
        ...
        accumulated_results = find_accumulated_match(
            ...
            used_array_indices, used_sheet_names,
        )
        ...
        for acc in accumulated_results:
            ...
            filtered, excluded = _filter_excluded(merged_products)
            if excluded:
                all_excluded_products.extend(excluded)
            if not filtered:
                continue
            ...
            try:
                ...
                detail['config'] = matrix_entry
                detail['variant_name'] = f"accumulated_{len(acc['selected_boms'])}boms"
                detail['bom_key'] = ''
                all_detail_results.append(detail)
                print(f"   ✅ JA accumulated detail: {detail['sheet_name']} "
                      f"(merged {len(acc['selected_boms'])} BOMs)")
            except Exception as e:
                import traceback
                print(f"   ❌ JA accumulated detail failed: {e}")
                traceback.print_exc()
    ...
    if pending_boms and has_explicit_matrix_arrays and matrix_array_entries:
        ...
        for acc in info_acc_results:
            ...
            try:
                _info_acc_array = dict(first_entry)
                _info_acc_array['table_qty'] = bom_base
                detail = create_detail_sheet(
                    master_wb,
                    _info_acc_array,
                    bracket_prods,
                ...
                detail['variant_name'] = f"info_accumulated_{len(m_indices)}entries"
                detail['bom_key'] = ''
                all_detail_results.append(detail)
                print(f"   ✅ JA info-accumulated detail: {detail['sheet_name']} "
                      f"({len(m_indices)} info entries → BOM base={bom_base})")
            except Exception as e:
                import traceback
                print(f"   ❌ JA info-accumulated detail failed: {e}")
                traceback.print_exc()
    ...
    if pending_boms and has_explicit_matrix_arrays and matrix_array_entries:
        ...
        remaining_info_indices = [
            ...
            if i not in used_array_indices
        ]
        ...
        for m_idx in remaining_info_indices:
            ...
            _indirect_array = dict(info_entry)
            try:
                detail = create_detail_sheet(
                    master_wb,
                    _indirect_array,
                ...
                detail['config'] = bom_config
                detail['variant_name'] = 'indirect'
                detail['bom_key'] = ''
                all_detail_results.append(detail)
                print(f"   ✅ JA indirect detail: {detail['sheet_name']} "
                      f"(info {info_rows}×{info_cols}_{info_qty} → BOM base={bom_base})")
            except Exception as e:
                import traceback
                print(f"   ❌ JA indirect detail failed: {e}")
                traceback.print_exc()
    ...
    if inverter_products_list and has_valid_inverter_qty:
        ...
        for p in inverter_products_list:
            vn = p.get('variant_name', '')
            if vn:
                variant_groups.setdefault(vn, []).append(p)
            else:
                no_variant.append(p)

        if no_variant:
            try:
                inv_filtered, inv_excluded = _filter_excluded(no_variant)
                if inv_excluded:
                    all_excluded_products.extend(inv_excluded)
                if inv_filtered:
                    detail_sheet_products.extend(inv_filtered)
                first_arr = arrays[0] if arrays else {}
                inv_array_info = {
                    'no': '0',
                    'rows': first_arr.get('rows', 0),
                    'cols': first_arr.get('cols', 0),
                    'table_qty': first_arr.get('table_qty', 1),
                    'note': '',
                }
                ...
                if inv_detail:
                    inv_detail['variant_name'] = ''
                    inv_detail['inv_remark'] = ''
                    inverter_detail_results.append(inv_detail)
                    print(f"   ⚡ パワコン明細シート生成: {inv_detail.get('sheet_name')}")
            except Exception as e:
                import traceback
                print(f"   ❌ パワコン明細シート生成失敗: {e}")
                traceback.print_exc()

        for vn, inv_prods in variant_groups.items():
            ...
            try:
                ...
                if matched_arr is None:
                    for arr in arrays:
                        ...
                        if arr_str in vn or vn.startswith(arr_str):
                            matched_arr = arr
                            break
                ...
                if inv_detail:
                    ...
                    inverter_detail_results.append(inv_detail)
                    print(f"   ⚡ パワコン明細シート生成: {inv_detail.get('sheet_name')} ({vn})")
            except Exception as e:
                import traceback
                print(f"   ❌ パワコン明細シート生成失敗 ({vn}): {e}")
                traceback.print_exc()
            ...
            if not pile_filtered:
                ...
                if not pile_filtered:
                    ...
                    for _pp in pile_filtered:
                        ...
                        _pp_pi = resolve_price_info(price_mapping, _pp_code) if price_mapping else None
                    ...
                    print(f"   🔩 地盤杭汇总: 数量={_pq}, 金額={_pa:.2f}")
        except Exception as e:
            import traceback
            print(f"   ❌ 地盤杭汇总失敗: {e}")
            traceback.print_exc()
    ...
    if all_detail_results:
        ...
        if return_details:
            ...
            for p in detail_sheet_products:
                ...
                if not code:
                    continue
                pi = resolve_price_info(price_mapping, code)
