...
询价价格查询模块（前端 `email_management.js`）用于在 BOM 报价生成后，对数据库中无价格匹配的物料（自动进入询价表）进行**案件级批量定价**。报价员在一个页面中按案件分组填写吨价/单价，系统自动换算各币种（美元/欧元/人民币外汇/人民币），保存后同步到价格缓存库，并通过钉钉工作通知提醒相关人员。

**核心文件**：

| 文件 | 作用 |
|------|------|
| `frontend/js/pages/email_management.js` | 询价价格查询前端页面（案件卡片、吨价汇总表、明细表） |
| `backend/services/inquiry_service.py` | 询价业务逻辑（保存吨价、汇率换算、钉钉通知、导出 Excel） |
| `backend/repositories/inquiry_repository.py` | 数据访问层（`ks_inquiry_items` 表 CRUD + 幂等迁移） |
| `backend/api/inquiry.py` | 询价相关 API 路由（Blueprint） |

### 14.2 数据库表：`ks_inquiry_items`

每条记录对应 BOM 中一个无价格匹配的物料行，保存案件级定价结果。

| 列名 | 类型 | 说明 |
|------|------|------|
| `id` | INTEGER PK | 自增主键 |
| `project_name` | TEXT | 案件/项目名称（分组键） |
| `material_code` | TEXT | 物料工程编码 |
| `name` / `spec` / `unit` / `quantity` | TEXT | 物料基本信息 |
| `unit_weight` / `total_weight` | REAL | 单重(kg)、总重量(kg) |
| `pricing_attr` | TEXT | 定价属性（D/M/F/Q/WTX/WTP/W），决定汇率类别 |
| `unit_price_usd` / `unit_price_eur` / `unit_price_cny` / `unit_price_rmb` | REAL | 换算后各币种单价 |
| `quotation_date` / `valid_until` | TEXT | 报价日期、有效期 |
| `status` | TEXT | 状态（`pending` 待报价 / `priced` 已报价） |
| `pricing_method` | TEXT | **报价方式**：`ton`（吨价）或 `unit`（单价） |
| `price_category` | TEXT | **手动选择类别**（定价属性未知时用，如 `dizhuang`/`lv`/`lvpj`/`tie`/`waigou`） |
| `discount` | TEXT | **折扣**（如 `95` 表示95折） |
| `mold_fee` | TEXT | **模具费** |
| `moq` | TEXT | **起订量**（Minimum Order Quantity） |
| `remark` | TEXT | **备注** |
| `inquirer` / `business_name` | TEXT | 询价人、业务员 |
| `preinstall` | TEXT | 预装/非预装标识 |

> `pricing_method`、`price_category`、`discount`、`mold_fee`、`moq` 五列为本次新增，通过 `inquiry_repository.py::_add_column()` 幂等迁移（已有库自动加列，不影响历史数据）。`shipping_company` 列保留在表中但已停用。

### 14.3 前端界面结构

页面按**案件**（`project_name`）分组展示，每个案件一张卡片（`_renderCaseCard`）：

```
┌─ 案件卡片 ───────────────────────────────────────────────────────────────┐
│ ▼ 案件：XXX项目  业务：张三  共 12 项  待报价 5  [保存本案件价格] [下载]  │
│                                                                          │
│ ┌─ 填写价格（吨价）──────────────────────────────────────── 有效期 [▾] ─┐│
│ │ ┌──────────┬──────┬──────┬──────┬──────┬──────┬──────┬──────┬───┐    ││
│ │ │物料编码  │吨价  │类别  │报价  │长度  │吨位  │总重量│ 价格 │折扣│... │    ││
│ │ │          │类型  │      │方式  │范围  │范围  │      │(公式)│    │    │    ││
│ │ ├──────────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼───┤    ││
│ │ │WTX-...   │铁    │[只读]│[▾吨价]│3+米 │0.5吨 │850kg│=5*4  │ 95│... │    ││
│ │ │WTP-...   │铁    │[只读]│[▾吨价]│1-3米 │0.5吨 │320kg│6800  │   │... │    ││
│ │ │ALU-...   │—     │[▾铝] │[▾单价]│  —   │0.1吨 │ 50kg│120   │   │... │    ││
│ │ └──────────┴──────┴──────┴──────┴──────┴──────┴──────┴──────┴───┘    ││
│ │ 表头列：物料编码/吨价类型/类别/报价方式/长度范围/吨位范围/总重量/价格   ││
│ │         /折扣/模具费/起订量/发货公司/备注                              ││
│ │                                              [保存本案件价格]        ││
│ └──────────────────────────────────────────────────────────────────────┘│
│                                                                          │
│ ┌─ 明细 ──────────────────────────────────────────────────────────────┐ │
│ │ # 物料编码 名称 规格 数量 单重 总重量 属性 美元 欧元 人民币外汇 人民币 折扣│ │
│ │ 1 WTX-...  ...  ...  10  0.5  5.0   F    1.2  1.1   7.8    8.5  95   │ │
│ │ ↑ 价格由上方汇总表统一计算；折扣随汇总表折扣自动同步                  │ │
│ └──────────────────────────────────────────────────────────────────────┘ │
│                                                                          │
│ ┌─ 案件备注 ──────────────────────────────────────────────────────────┐ │
│ │ 业务备注: ...    报价人员备注: ...    附件: [上传] [xxx.xlsx ⬇ 🗑]   │ │
│ └──────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────┘
```

#### 14.3.1 报价方式下拉（`pricing_method`）

每行吨价汇总表新增**报价方式**下拉框（`_renderTonRow`），值为 `吨价` 或 `单价`：

| 报价方式 | 说明 | 价格换算逻辑 |
|---------|------|-------------|
| **吨价** (`ton`) | 铁类物料默认 | `base = 单重(kg) × 吨价(元)` → ÷汇率÷点数换算其他币种 |
| **单价** (`unit`) | 铝/铝配件/地桩/外购件默认 | `base = 输入值(元)` → ÷汇率÷点数换算其他币种 |

- 铁类物料（`pricing_attr` 为 `WTX`/`WTP`）默认选中**吨价**；其他类别默认选中**单价**；均可手动切换。
- 切换报价方式时，价格输入框 placeholder 随之变化（`基准吨价` / `成本(元)`），并立即重新换算明细价格。
- 前端 `_computeDetPrices(uw, price, category, pricingMethod)` 函数：`pricingMethod === 'ton'` 时按 `单重×吨价` 换算，否则直接使用输入值。

#### 14.3.2 类别下拉（`price_category`）

当物料的定价属性在数据库中查不到时（`group.category` 为空），类别单元格显示**下拉选择框**（`<select class="cat-select">`），选项为：

| 选项 | key | 对应汇率/点数 |
|------|-----|-------------|
| —选择— | `''` | 无（价格为空，无法换算） |
| 地桩 | `dizhuang` | 地桩类汇率/点数 |
| 铝 | `lv` | 铝类汇率/点数 |
| 铝配件 | `lvpj` | 铝配件汇率/点数 × 铝配件系数 |
| 铁 | `tie` | 铁类汇率/点数 |
| 外购件 | `waigou` | 外购件汇率/点数 |

- 定价属性已知的物料，类别单元格显示**只读文本**（如 `F`、`M`）。
- 选择类别后自动重新套用报价方式默认值（铁→吨价，其他→单价）。
- 有效类别 = `_effCategory(group, st)` = `st.category_override || group.category`，即手动选择优先于自动推断。

#### 14.3.3 四个新增列（折扣/模具费/起订量/备注）

吨价汇总表新增四列输入框（`_renderTonRow`），均为**组级**（同一物料编码+长度档共享）：

| 列 | HTML class | data-extra 值 | 说明 |
|----|-----------|--------------|------|
| 折扣 | `.ton-extra` | `discount` | 如 `95` 表示95折；变更时同步到明细表折扣列 |
| 模具费 | `.ton-extra` | `mold_fee` | 模具费用文本 |
| 起订量 | `.ton-extra` | `moq` | 最低起订量 |
| 备注 | `.ton-remark` | `remark` | 附加备注 |

- **折扣同步明细**：填写折扣后，下方明细表对应行的折扣列（`.det-discount`）实时更新（`_refreshDetailPricesForGroup`）。
- 保存时，组级字段通过 `update_inquiry_item` 传播到该组的每条明细记录。

#### 14.3.4 案件级有效期

吨价区域标题右侧新增**有效期下拉**（`data-case-valid`），选择天效（如 3天/7天/14天/30天/90天）。

- 前端 `state.caseValidUntil[project]` 存储选择值，保存时随 `_collectCaseEntries` 传给后端。
- 保存时每条明细的 `valid_until` 统一使用案件级有效期（`entry.get('valid_until') or case_valid`）。

#### 14.3.5 Tab 键导航

吨价汇总表中的所有输入框（`.ton-input`、`.formula-input`、`input[data-extra]`）支持 **Tab 键纵向跳转**：

- 在当前行（`tr[data-ton-row]`）内按 Tab，焦点跳到**下一行同列**的输入框。
- 最后一行按 Tab 时，不做拦截，按浏览器默认行为跳转。
- 实现位置：`_bindCaseEvents` 中的 `keydown` 事件处理器。

#### 14.3.6 保存按钮

每个案件卡片的吨价区域底部新增**保存按钮**（`data-case-save`），与案件标题栏的保存按钮功能相同（`saveCaseTonPrices`）。

- 案件金额保存后 7 天内可修改；超过 7 天自动锁定（`_isCaseLocked`），按钮显示为「⛔ 已锁定」。

### 14.4 后端保存流程（`save_case_ton_prices`）

```
POST /api/inquiry/case-ton-prices
  { project_name, valid_until, entries: [...], pricer_remark }

  ┌─────────────────────────────────────────────────────────────────────┐
  │ 1. 案件锁定检查                                                      │
  │    → _is_case_locked(project_name)                                   │
  │    → 超过 7 天 → 返回 403 { locked: true }                           │
  └─────────────────────────────────────────────────────────────────────┘
                               │
  ┌─────────────────────────────────────────────────────────────────────┐
  │ 2. 汇总各组吨价 → 写入临时价格设置（ks_temp_price_settings）         │
  │    → ton_{type}_int_{长度档}_{吨位档} = 吨价基准值                    │
  └─────────────────────────────────────────────────────────────────────┘
                               │
  ┌─────────────────────────────────────────────────────────────────────┐
  │ 3. 逐组换算各币种单价                                                │
  │    → 查询该组物料定价属性 → 确定汇率类别 cat                          │
  │    → pricing_method == 'ton': base = 单重 × 吨价 × 规格系数           │
  │    → pricing_method == 'unit': base = 输入值                          │
  │    → cat == 'lvpj': base × 铝配件系数(默认1.03)                       │
  │    → 美元 = base ÷ 汇率 ÷ 点数（地桩仅÷汇率）                        │
  │    → 人民币 = base（原始值）                                          │
  │    → 每条明细写入: 单价×4币种 + 报价日期 + 有效期                      │
  │                    + 报价方式 + 类别 + 折扣/模具费/起订量/发货公司/备注 │
  │                    + status='priced'                                  │
  └─────────────────────────────────────────────────────────────────────┘
                               │
  ┌─────────────────────────────────────────────────────────────────────┐
  │ 4. 同步价格缓存（ks_inquiry_price_cache）+ 吨价历史（ks_ton_price_   │
  │    history，保存1年）+ 案件锁定时间戳                                 │
  └─────────────────────────────────────────────────────────────────────┘
                               │
  ┌─────────────────────────────────────────────────────────────────────┐
  │ 5. 钉钉通知当前报价员                                                 │
  │    → _notify_case_priced(project_name, updated_count)                │
  │    → 推送 Markdown 消息到报价员钉钉                                   │
  └─────────────────────────────────────────────────────────────────────┘
```

### 14.5 钉钉通知格式

保存成功后，向当前登录用户的钉钉发送工作通知（`_notify_case_priced`），格式如下：
...
3、日期：2026-06-23 10:30

4、备注：客户要求月底前交付            ← 手写备注（价格公式部分不外发）

5、物料明细：                           ← 仅含有备注/起订量/模具费的物料

- WTX-001：起订量 500 / 模具费 3000元 / 备注 需热镀锌

- ALU-002：起订量 200 / 备注 阳极氧化

已填写 12 条价格，请及时查看。

[点此查看](http://192.168.x.x:5000/frontend/...)
```

- 物料明细由 `_build_material_notify_lines(project_name)` 生成：查询该案件所有明细项，按物料编码去重，仅输出有备注/起订量/模具费的物料。
- 手写备注与公式备注以 `【价格公式备注】` 分隔，仅外发手写部分。

### 14.6 案件价格导出（`build_case_price_workbook`）

点击「下载吨价/价格」导出 Excel，包含三个 Sheet：

| Sheet | 名称 | 内容 |
|-------|------|------|
| 1 | **吨价** | 物料编码/名称/吨价类型/长度范围/吨位范围/总重量/吨价(基准)/有效期/各币种价格 |
| 2 | **明细生成价格** | 序号/物料编码/名称/规格/数量/单重/总重量/单位/各币种价格/有效期/**报价方式/类别/折扣/模具费/起订量/发货公司/备注/保存备注** |
| 3 | **案件信息** | 项目名/业务备注/报价人员备注/业务员/附件列表 |

Sheet 2 的「保存备注」列为组合格式，便于人工查阅：

```
物料编码-规格-长度范围-吨位范围-折扣-模具费-起订量-发货公司-备注
```

例如：`WTX-001-L2000-0.5吨-95-3000-500-苏州科盛-需热镀锌`

### 14.7 汇率类别与报价方式的关系

| 概念 | 控制对象 | 取值来源 | 影响 |
|------|---------|---------|------|
| **汇率类别** (`cat`) | 换算时使用哪套汇率/点数 | 定价属性自动推断（`_ATTR_TO_CAT`），或手动选择（`price_category`） | 地桩÷汇率；其他÷汇率÷点数；铝配件×系数 |
| **报价方式** (`pricing_method`) | 基准价格的计算方式 | 铁默认 `ton`，其他默认 `unit`，可手动切换 | `ton`：`base = 单重 × 吨价`；`unit`：`base = 输入值` |

两者正交：类别决定**汇率换算口径**，报价方式决定**基准价格的计算方法**。

### 14.8 相关 API 接口

| 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|
| GET | `/api/inquiry/items` | 询价项列表（支持案件/状态/分页过滤） | quotation |
| POST | `/api/inquiry/case-ton-prices` | 按案件批量保存吨价/单价 | quotation |
| GET | `/api/inquiry/case-price-export` | 下载案件吨价+价格 Excel | quotation |
| GET | `/api/inquiry/case-lock` | 查询案件锁定状态 | quotation |
| GET | `/api/inquiry/temp-settings` | 读取临时价格设置（吨价/汇率/点数/系数） | quotation |
| POST | `/api/inquiry/temp-settings` | 更新临时价格设置 | quotation |
| GET | `/api/inquiry/ton-history` | 吨价历史回溯 | quotation |
| POST | `/api/inquiry/case-meta` | 保存案件备注/附件元信息 | quotation |
